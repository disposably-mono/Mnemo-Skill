...
them into Anki. You (Claude) are the intelligent middle of the pipeline: you read
the material and produce well-formed **Facts**; deterministic Python scripts handle
parsing and the Anki import.

**Never invent multiple-choice cards or in-card JavaScript.** Cards are recall-first
(cloze / basic / overlapping-cloze lists). Distractors, when useful, are shown on the
*answer* side as "common confusions" — they are study aids, not clickable options.
...
3. **Generate Facts.** Apply the card-quality rules below. Emit Facts conforming to
   ...
   `cards/<session>.md` and a machine `cards/<session>.jsonl`.
...
5. **Import.** Run `python scripts/import_cards.py cards/<session>.jsonl`. It maps each
   Fact to the target note type (`mappings.toml`, default = MONO types), pushes to Anki
...
- Recall-first only: no MCQ note types, no in-card JS.
- Always go through the review gate before importing.
- Default note types are the bundled MONO types; respect any override in `mappings.toml`.
- If AnkiConnect is unreachable, the importer writes a `.apkg` — tell the user where it is
  and how to import it manually.
