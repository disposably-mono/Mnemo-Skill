...
| Format variety | Evidence supports retrieval and appropriate representations, not a universal requirement for three note types per deck. | Variety can reduce repetitive cueing, but duplicate cards can also create workload. | Require at least three types across a deck when at least three atomic facts exist; do not duplicate a single fact merely to meet the quota. |
...
| At least 3 formats | `diversify_card_types()` assigns Q&A, cloze, and reverse when enough facts exist. | `FORMAT_VARIETY` |
| Relevant image | Source image becomes `image-supported`; no image is invented. | `TEXT_ONLY_DECK`, `IMAGE_ALT` |
