"""Bundled font resolution and media-path dedup, ported from the pre-rewrite package."""
...
def bundled_font_paths() -> list[Path]:
    ...
    return sorted(
        _extracted_font_path(path) for path in font_root.iterdir() if path.name.endswith(".ttf")
    )


def _extracted_font_path(resource) -> Path:
...
def unique_media_paths(paths: Iterable[Path]) -> list[Path]:
    ...
    for raw_path in paths:
        ...
        if existing is None:
            ...
            raise ValueError(
                f"multiple media files use the filename {path.name!r}: {existing} and {path}"
            )
